...
void lu_par_dag(Matrix A, info_type info){
  ...
  ProgTable ptable; /* tableau avancement : ce qui a été fait jsq'a present */
  Task      task;
  int i;
...
#pragma omp parallel private(task)
{
  for(;;){
    ...
    #pragma omp critical /* si pas de nom aux régions critique =
    comme si toutes le meme nom > ne peuvent pas etre executees simultanement
    toutes les 3 */
    {
...
/* peut changer les opé données pour etre plus efficace :
oracle renvoie en prio la modif d'un panel plutot que un udpate (chemin le 
plus long > prend plus de temps, tous les panel sur chemin le plus
long ) : sépare les deux boucles de decisions panel / update */
