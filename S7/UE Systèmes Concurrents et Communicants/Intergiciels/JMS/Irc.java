...
class writeListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		try {
			...
			TextMessage mess = Irc.session.createTextMessage();
			mess.setText(msg);
			Irc.producer.send(mess);
...
class leaveListener implements ActionListener {
	public void actionPerformed (ActionEvent ae) {
		try {
			...
			// envoi msg de deco pour etre supp des users des autres 
			Irc.print("deconnecte");
			System.exit(0);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
