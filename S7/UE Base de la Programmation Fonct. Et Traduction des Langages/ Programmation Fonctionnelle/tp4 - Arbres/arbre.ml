...
(* signature  : ajout : 'a list -> 'a arbre -> 'a arbre 
   paramètres : - une liste d'éléments (caractères dans le cas d'un dico)
                - un arbre n-aire 
   résultat : arbre avec le mot retiré *)

let rec retrait_arbre lc (Noeud(b, l_br)) = 
match lc with
|[] -> Noeud(false,l_br)
|c::qlc -> let br = recherche c l_br in 
            match br with 
            |None -> Noeud(b, l_br)
            |Some a -> retrait_arbre qlc a ;;

let%test _ = appartient_arbre ['l';'a';'i';'t'] (retrait_arbre ['l';'a';'i';'t'] arbre_sujet) = false

(* fonction qui renvoie la liste des elmts dans l'ordre lexicographique
dico_arbre : 'a arbre -> 'a list list 
param : arbre et liste des mots 
*)

let rec dico_arbre (Noeud(b,l_br)) =
match l_br with
|[] -> if b then [[]] else [];
|(c,a) :: ql_br -> 
   let mots_suiv = List.map (fun l -> c::l) (dico_arbre a) in
   if b then []::mots_suiv@(dico_arbre (Noeud(false,ql_br)))
   else mots_suiv@(dico_arbre (Noeud(b,ql_br))) ;;

(*renvoie un arbre sans branches inutiles*)
(*let rec elimine_br (Noeud(b,l_br)) =
let rec op_a lbr = 
match lbr with 
|[] -> if b then Some lbr else None
|(c,arbre)::qlbr -> 

in match op_a l_br with 
|None -> if b Noeud(b,[])
|Some (c,a) -> Noeud(b,(c,a))
*)

(*let rec elimine_br (Noeud(b,lbr)) =
let rec elim_aux bol lbr = 
match lbr with 
|[] -> if bol then Some [] else None
|(c,a)::q -> *)
