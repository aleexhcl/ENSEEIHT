...
|Leaf _ -> 0
|Node(b1,b2) -> 1 + max (height b1) (height b2)
...
|Leaf _ -> 1
|Node(b1,b2) -> 1 + num_nodes b1 + num_nodes b2
...
|Leaf f -> if f = _0 then 0 else 1
|Node(b1,b2) -> num_values b1 + num_values b2
 ...
 *)
let rec bits n addr = match (n,addr) with
|0,_ -> []
|k,ad -> (ad mod 2)::(bits (k-1) (ad/2))
...
match (tr,addrl) with
|Node(_,_), [] -> failwith "adresse supérieure à la profondeur de l'arbre"
|Leaf a , _ -> a
|Node(b1, b2), t::q -> if t = 0 then search b1 q else search b2 q
...
List.fold_right (fun t spq -> if t = 0 then Node(spq, Leaf _0) 
                                else Node(Leaf _0, spq)) addrl (Leaf x)
...
|Node(_,_), [] -> failwith "adresse supérieure à la profondeur de l'arbre"
|Leaf _, [] -> Leaf x
(*si on tombe sur une feuille alors que l'adresse n'est pas finie, c'est une feuille vide, 
on écrit Leaf _ pour le matching, mais le seul possible est Leaf _0*)
|Leaf _, t::q -> if t = 0 then Node(sprout x q,Leaf _0) else Node(Leaf _0,sprout x q) 
|Node(b1,b2),t::q -> if t = 0 then Node(update b1 x q,b2) else Node(b1,update b2 x q)
