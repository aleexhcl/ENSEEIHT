...
struct
     ...
     *)
    let allocsize (bs, tr) = height tr * bs
    ...
    let write (bs, tr) addr value = if busyness (bs, tr) + addr < size (bs, tr) then (bs,update tr value (bits bs addr)) else raise OutOfBound
end
